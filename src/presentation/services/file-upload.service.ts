...
export class FileUploadService{
   ...
   async uploadMultiple(filesRequest:UploadedFile[],folder:string='uploads',validExtensions:string[]=['png','jpg','jpeg','gif']){

        const files = await Promise.all(
            filesRequest.map(file=>this.uploadSingle(file,folder,validExtensions))
        )
        return files;
    }   
